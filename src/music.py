...
def m_play(*ch_s):
    d2 = 0
    for ch in ch_s:
        d2 |= (1 << (ch - 1)) if ch > 0 and ch < 26 else 0
    x68k.iocs(x68k.i.OPMDRV, d1=0x08, d2=pack('L', d2), d3=0, d4=0)

def m_stop(*ch_s):
    d2 = 0
    for ch in ch_s:
        d2 |= (1 << (ch - 1)) if ch > 0 and ch < 26 else 0
    x68k.iocs(x68k.i.OPMDRV, d1=0x0a, d2=pack('L', d2), d3=0, d4=0)

def m_cont(*ch_s):
    d2 = 0
    for ch in ch_s:
        d2 |= (1 << (ch - 1)) if ch > 0 and ch < 26 else 0
    x68k.iocs(x68k.i.OPMDRV, d1=0x0b, d2=pack('L', d2), d3=0, d4=0)

def m_stat(ch=0):
    stat =  x68k.iocs(x68k.i.OPMDRV, d1=0x09, d2=0, d3=0, d4=0)
    return stat if ch <= 0 or ch > 25 else 1 if stat & (1 << (ch - 1)) else 0 
